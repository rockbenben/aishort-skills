...
| 12 | **App aborts on launch on some Linux desktops** (Xfce, VMs, hybrid/NVIDIA graphics) with `Could not create surfaceless EGL display: EGL_BAD_ALLOC. Aborting...` — WebKitGTK 2.42+'s default DMABUF renderer fails to init the surfaceless EGL display on those GPU/driver combos and kills the process. (The `xapp-gtk3-module` / `atk-bridge` lines around it are harmless GTK/at-spi noise.) **Two-stage symptom — read the signature to know which fix you need:** *stage 1* (no fix) = the EGL line is followed by `Abandon`/`core dumped`, process dies, **no window**. *stage 2* (DMABUF already disabled but the GPU still can't init WebKit's **GL compositing** context — confirmed on Manjaro Xfce) = same EGL line but **no core dump**; the window **opens blank** and you can close it normally. NOT a routing/white-screen (gotcha #1): EGL errors are present, it's platform-specific, and Windows renders fine. | Set `WEBKIT_DISABLE_DMABUF_RENDERER=1` at the **very top of `run()`, before `Builder::default()`** (must precede GTK/WebView init), gated on `#[cfg(target_os = "linux")]` and on the var being unset (`var_os(...).is_none()`) so you don't override a user/distro value. This is GPU-buffer **sharing** only — still GPU-accelerated, near-zero visual cost, and fixes stage 1. **For stage 2 add `WEBKIT_DISABLE_COMPOSITING_MODE=1`** in the same only-if-unset block — it forces *software* rendering (janky animations, broken `transform`/`filter`/`backdrop-filter`), so don't reach for it blind: **confirm first** by having the reporter run the existing build with both vars set inline — `WEBKIT_DISABLE_DMABUF_RENDERER=1 WEBKIT_DISABLE_COMPOSITING_MODE=1 ./App.AppImage` — and only bake it in once the UI paints. Decision rule: animation-heavy UI → keep it opt-in (env var / docs only); **non-animation tool** (diff/translate/calculator) → defaulting it on for all Linux is a fine reliability-over-polish call (still only-if-unset). Tauri has no formal docs stance but [acknowledges this var as the workaround (issue #9394)](https://github.com/tauri-apps/tauri/issues/9394); the "inject only if unset" pattern is the ecosystem norm (yaak, meetily). See `tauri-files.md` `lib.rs`. |
